...
df['Tokenized'] = df['Deskripsi'].apply(lambda x: vectorizer.build_tokenizer()(x))

# Stemming
df['Stemmed'] = df['Tokenized'].apply(lambda x: [stemmer.stem(word) for word in x])

# Display tokenized and stemmed data
print(df[['Deskripsi', 'Tokenized', 'Stemmed']])

# You can add more logic here to classify reviews as positive, neutral, or negative based on the tokenized and stemmed data
